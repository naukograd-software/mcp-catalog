...
// ApplyToCLI generates config snippets for various CLI tools
func (m *Manager) ApplyToCLI(tool string) (string, error) {
	cfg := m.store.Get()

	switch tool {
	case "claude":
		return m.generateClaudeConfig(cfg)
	case "codex":
		return m.generateCodexConfig(cfg)
	case "gemini":
		return m.generateGeminiConfig(cfg)
	case "kilo", "koli":
		return m.generateGenericConfig(cfg, tool)
	case "open-code":
		return m.generateOpenCodeConfig(cfg)
	default:
		return m.generateGenericConfig(cfg, tool)
	}
}

func (m *Manager) generateClaudeConfig(cfg *config.Config) (string, error) {
	out := map[string]any{
...
func (m *Manager) generateCodexConfig(cfg *config.Config) (string, error) {
	out := map[string]any{
		"mcpServers": cfg.MCPServers,
	}
	data, err := json.MarshalIndent(out, "", "  ")
...
func (m *Manager) generateGeminiConfig(cfg *config.Config) (string, error) {
...
func (m *Manager) generateOpenCodeConfig(cfg *config.Config) (string, error) {
	servers := make(map[string]any)
	for name, srv := range cfg.MCPServers {
		if !srv.Enabled {
			continue
...
func (m *Manager) generateGenericConfig(cfg *config.Config, _ string) (string, error) {
	out := map[string]any{
		"mcpServers": cfg.MCPServers,
	}
	data, err := json.MarshalIndent(out, "", "  ")
